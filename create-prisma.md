...
生成するコードはPython 3.xとし、Pandas, re, json, os, ioをインポートすること。また、pipで一般的に入手可能なパッケージは積極的に使用することで、不必要なコーディングを削減すること。
...
* **処理内容**:
  ...
  * インデックス(`@@index([])`)を定義する。但し、`@unique`あるいは`@@unique([])`と同じ項目並びのインデックスは定義しない。
    * インデックスがB-Treeの場合、ソート順を適用する。ソート順はバリデーションにて`ASC` `DESC`のいずれか(または両方)が設定されている前提である。
    * インデックスがHashの場合、`type: Hash`の設定を行う。
...
* **ログ出力**:
  * 処理の経過をコンソールと`logs/conversion_info.log`ファイルの両方に出力する。これにより、処理の経過の確認を支援する。
  * 発生したワーニングメッセージをコンソールと`logs/conversion_warnings.log`ファイルの両方に出力する。これにより、問題の迅速な特定とデバッグを支援する。
  * 発生したエラーメッセージをコンソールと`logs/conversion_errors.log`ファイルの両方に出力する。これにより、問題の迅速な特定とデバッグを支援する。
