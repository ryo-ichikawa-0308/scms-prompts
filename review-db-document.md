...
- 各サブセクションは、下記の項目を持つ表形式であること。
  ...
  - テーブル論理名 (リンク)
    ...
    - 個別設計書へのリンクとして、[テーブル名](path)の形式であること。リンクが貼られていない場合は警告として指摘せよ。
...
- 参照元テーブルの「テーブル作成順序」が、参照先テーブルの「テーブル作成順序」より大きい値であることを確認せよ。
- 参照先テーブル(親)の「テーブル作成順序」が、参照元テーブル(子)の「テーブル作成順序」より小さい値であることを確認せよ。
...
- 下記の主キー及び監査項目が定義されており、定義(カラム論理名、カラム物理名、型、PK、FK、UNIQUE、NOTNULL、DEFAULT)が下記の通り厳密に定義されていることを確認せよ。
  - 備考欄は自由記述であるため、「ID」「更新日時」「更新者」の備考欄が未記載の場合は助言、記載内容誤りの場合は警告として指摘せよ。
...
- カラム物理名が、カラム論理名を英訳した英単語単数形で命名されていることを確認せよ。但し、人手チェックとするため、違反がある場合は警告として指摘せよ。
- カラム物理名が、全て英単語の単数形かつsnake_case形式で記述されていることを確認せよ。
...
| `INTEGER`       | 不可              | **Int**      |                                                                                                              |
| `BIGINT`        | 不可              | **BigInt**   |                                                                                                              |
| `REAL`          | 不可              | **Float**    | 精度が失われる可能性があるため、`DECIMAL(P, S)`に変更することを助言せよ。                                    |
| `FLOAT(N)`      | 不可              | **Float**    | 精度が失われる可能性があるため、`DECIMAL(P, S)`に変更することを助言せよ。                                    |
...
- 「カラム定義」で`UKn`が設定されているカラムの組み合わせと、インデックス定義が重複していないことを確認せよ。
